...
contract NFTPoolBurnAndMint  is CCIPReceiver, OwnerIsCreator {
    ...
    error FailedToWithdrawEth(address owner, address target, uint256 value); // Used when the by the contract owner.
    ...
    function burnAndSendNFT(
        ...
        address receiver) public returns (bytes32 messageId){
            //将NFT转移到这个pool并锁定
            //transferForm是ERC72的方法,将原有人所掌握的nft(tokenId)转移到这个pool地址上,这样nft就锁定到池子里了
            wnft.transferFrom(msg.sender,address(this),tokenId);
            //烧掉
            wnft.burn(tokenId);
            //因为传送数据不能用很复杂的数据类型,一般都是转换成bytes
            bytes memory payload = abi.encode(tokenId,newOwner);
            //调用sendMessagePayLINK方法,发送数据
